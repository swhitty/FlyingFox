...
> Note: iOS will hangup the listening socket when the app is suspended in the background. Once the app returns to the foreground, `HTTPServer.start()` detects this, throwing `SocketError.disconnected`. The server then needs to be started once more.
